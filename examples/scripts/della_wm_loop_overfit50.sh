...
# ============================================================================
# WM FINE-TUNE OVERFIT EXPERIMENT — 50-TRAJ VARIANT (no pretrain mixing)
# ----------------------------------------------------------------------------
# Same diagnostic harness as della_wm_loop_overfit.sh, but with a LARGER
# online buffer and NO pretrain-replay mixing. Use this to test the
# hypothesis "drift comes from too few anchor points, not from missing
# pretrain anchors": if 50 collected trajs is enough to stabilize fine-
# tuning, we don't need the pretrain-mix path for real runs.
#
# What this script disables (same as the 2-traj variant):
#   * trajectory scoring (SCORED_PER_ROUND=0) — all trajs route via
#     .wm_only into the WM finetune buffer.
#   * SAC / policy updates (MULTI_GRAD_STEP=0, BASE_POLICY_PROB=1.0) —
#     every rollout is pure π₀; the SAC agent stays at init.
# What changes from the 2-traj overfit script:
#   * MAX_TRAJS=50  (was 2). Collection takes ~25× longer; budget time.
#   * WM_BUFFER_SIZE=50, WM_BUFFER_FREEZE_AT=50 — buffer holds all 50.
#   * WM_PRETRAIN_NUM_TRAJS=0, WM_MIX_ONLINE=1.0 — pretrain mix disabled.
#   * WM_GRAD_STEPS=500 (was 200) — more steps per cycle so each of the
#     50 trajs gets a few windows of gradient per cycle.
#   * WM_CHECKPOINT_EVERY=5 (was 1) — each ckpt is ~9 GB; checkpoint
#     fewer cycles to save disk.
#   * WM_SANITY_NUM_TRAJS=4 (was 2) — render a few more sample trajs so
#     "after" quality across the buffer is visible (still way less than
#     50 to keep the rendering time bounded).
# Expected outcome: more buffer diversity → less drift. The "after"
# panels should track GT across many cycles without the sharp fidelity
# collapse seen in the 2-traj run. If they still collapse, the issue
# isn't buffer size — it's base-model OOD-ness on π₀-collected data.
# ============================================================================
...
MAX_TRAJS=50                      # collect 50 trajs (the buffer freezes here),
                                  # then exit. Server self-loops fine-tune cycles
                                  # on those 50 trajs forever (see WM_SELF_LOOP).
                                  # Collection takes ~25× longer than the 2-traj
                                  # variant; budget walltime accordingly.
MAX_STEPS=500000
...
WM_GRAD_STEPS=500             # 2.5× the 2-traj variant: with 50 trajs and bs=4,
                              # this gives ~40 windows/traj per cycle.
WM_BATCH_SIZE=4
WM_LR=2e-5                     # same as 2-traj variant. Bump cautiously; large LR
                               # was what blew up fidelity on small buffers.
WM_MAX_GRAD_NORM=1.0
WM_BUFFER_SIZE=50              # hold all 50 collected trajs
WM_BUFFER_FREEZE_AT=50         # freeze once the 50th lands
WM_CHECKPOINT_EVERY=5          # checkpoints are ~9 GB each; save every 5 cycles

# ----- Pretrain-replay mix (OFF for this variant) -----
# This script tests whether ONLINE-ONLY fine-tuning is stable when the
# buffer is moderately sized (50 trajs). Keep WM_MIX_ONLINE=1.0 (and
# WM_PRETRAIN_NUM_TRAJS=0) so no pretrain anchoring happens. If you want
# the mix-on variant, use della_wm_loop_overfit.sh instead.
WM_PRETRAIN_ROOT="${WM_PRETRAIN_ROOT:-$WM_DATASET_ROOT}"
WM_PRETRAIN_SUITE="${WM_PRETRAIN_SUITE:-$TASK_SUITE}"
WM_PRETRAIN_NUM_TRAJS="${WM_PRETRAIN_NUM_TRAJS:-0}"
WM_MIX_ONLINE="${WM_MIX_ONLINE:-1.0}"
...
WM_SANITY_CHECK=1
WM_SANITY_NUM_TRAJS=4          # a few more samples than the 2-traj variant so
                               # you can see how "after" quality varies across
                               # the buffer.
WM_SANITY_WINDOWS=8
...
DATE_DIR=$(date +%m%d)
TIME_TAG=$(date +%H%M%S)
JOB_TAG="${SLURM_JOB_ID:-local}_${TIME_TAG}_overfit50"
...
echo "[full] on the frozen 50-traj buffer. Streaming server log below;"
