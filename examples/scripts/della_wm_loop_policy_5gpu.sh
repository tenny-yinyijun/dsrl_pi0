...
# ============================================================================
# POLICY-ONLY STABILITY TEST (frozen WM) — 5-GPU PARALLEL-SCORING VARIANT
# ----------------------------------------------------------------------------
# Same experiment as della_wm_loop_policy.sh, but scores trajectories in
# parallel across NUM_REWARD_WORKERS reward_server processes (one per
# REWARD_GPU). All N workers share the same $REWARD_ROOT/requests/ and
# $REWARD_ROOT/scores/ directories; concurrency is safe because the server
# atomically claims each .req via rename before processing (see
# reward_server.py::_try_claim — POSIX rename is atomic).
#
# Default layout (5 GPUs total):
#   REWARD_GPUS = 0,1,2,3   (4 GPUs scoring the WM in parallel)
#   TRAINER_GPU = 4         (1 GPU running SAC / data collection)
#
# With ~10s/window single-GPU scoring, 4-way parallel scoring drops a
# 50-window round from ~500s to ~125s — the per-round bottleneck moves
# from WM scoring back toward rollout collection.
#
# Each worker process loads its own copy of the WM (~9 GB GPU memory), so
# 4 workers consume ~36 GB across the 4 reward GPUs. Each process also
# loads its own LPIPS net (small).
#
# WM fine-tuning is DISABLED in this script (same as the single-GPU
# variant). If you want to enable it you'd need to coordinate weight
# updates across workers (currently each WMFineTuner would update its own
# local copy in isolation — fine for a smoke test, not for production).
# ============================================================================
...
DATE_DIR=$(date +%m%d)
TIME_TAG=$(date +%H%M%S)
JOB_TAG="${SLURM_JOB_ID:-local}_${TIME_TAG}_policy_5gpu"
...
cat > "$CONFIG_PATH" <<EOF
{
  "meta": {
    "job_tag": "$JOB_TAG",
    "experiment_type": "policy_only_5gpu_parallel_scoring",
