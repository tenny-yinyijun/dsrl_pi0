#!/bin/bash
# DSRL-π₀ + WM-reward loop on TWO GPUs (e.g. 2× L40), interactive use only.
#
# Topology:
#   GPU 0 -> reward server (SVD + CtrlWorld + LPIPS)        REWARD_GPU=0
#   GPU 1 -> dsrl_pi0 collector + trainer (Pi0 + SAC + env) TRAINER_GPU=1
#
# Usage: from inside a 2-GPU interactive allocation,
#     bash examples/scripts/run_wm_loop_2gpu.sh
#
...
# Two-GPU mode: reward server on GPU 0, trainer on GPU 1.
...
# Sanity check: 2-GPU mode requires distinct GPUs and at least 2 visible.
if [ "$REWARD_GPU" = "$TRAINER_GPU" ]; then
    echo "[run_wm_loop] FATAL: REWARD_GPU and TRAINER_GPU are both $REWARD_GPU."
    echo "[run_wm_loop] for 2-GPU mode they must be different (use run_wm_loop.sh for shared-GPU mode)."
    exit 1
fi
NUM_GPUS=$(nvidia-smi -L 2>/dev/null | wc -l || echo 0)
if [ "$NUM_GPUS" -lt 2 ]; then
    echo "[run_wm_loop] FATAL: only $NUM_GPUS GPU(s) visible, need 2."
    echo "[run_wm_loop] check your sbatch --gres / salloc allocation."
    exit 1
fi
...
# ---------------------------------------------------------------------------
# Start reward server in the background (on REWARD_GPU)
# ---------------------------------------------------------------------------
echo "[run_wm_loop] starting reward server on GPU $REWARD_GPU (logs -> $SERVER_LOG)"
(
    cd "$OPEN_WORLD_ROOT"
...
# ---------------------------------------------------------------------------
# Run the dsrl_pi0 collector + trainer (foreground, on TRAINER_GPU)
# ---------------------------------------------------------------------------
echo "[run_wm_loop] starting trainer on GPU $TRAINER_GPU..."
cd "$DSRL_ROOT"
